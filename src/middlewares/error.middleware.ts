...
export class SensError extends Error {
  public readonly status: number;
  public readonly expiryTimestamp: Date;

  constructor(status: number, message: string, expiryTimestamp: Date) {
    super(message);
    this.status = status;
    this.expiryTimestamp = expiryTimestamp;
    Object.setPrototypeOf(this, SensError.prototype);
  }
}
...
export class GlobalExceptionFilter implements ExceptionFilter {
  ...
  async catch(exception: any, host: ArgumentsHost) {
    ...
    if (exception instanceof CommonError) {
    ...
    } else if (exception instanceof SensError) {
      status = exception.status;
      message = exception.message;
      const expiryTimestamp = exception.expiryTimestamp;
      ...
      //예외 에러 DB log
      // await this.errorLogService.saveErrorLog(exception.stack);
